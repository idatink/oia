...
/* Signed, opaque tokens for patient match-room links (/matches/<token>).
   Mirrors packages/web/src/lib/matchToken.ts EXACTLY (same payload shape, same
   secret precedence, same HMAC + 22-char signature) so a token minted here on the
   dashboard verifies on the web app. Kept in @nia/shared so both services mint
   and verify identically. MATCH_TOKEN_SECRET is unset in practice, so both fall
   back to the shared WHATSAPP_INTAKE_SECRET. */
